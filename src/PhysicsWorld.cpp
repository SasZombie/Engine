...
void sas::PhysicsWorld::ApplyForces(Body &obj) const noexcept
{
    ...
    obj.kinematics->acceleration.x = (dragForceX * obj.kinematics->inverseMass);
}
...
void sas::PhysicsWorld::Integrate(Body &obj, float dt) const noexcept
{
    ...
    obj.kinematics->velocity.x += obj.kinematics->acceleration.x * dt;
    obj.transform.position.x += obj.kinematics->velocity.x * dt;
}
...
void sas::PhysicsWorld::ResolveConstraints(Body &obj, float dt) const noexcept
{
    ...
    ResolveBroadLower(obj, obj.shape.radius);
}
